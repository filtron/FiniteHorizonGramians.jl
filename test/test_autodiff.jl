    ...
    @testset "ForwardDiff.jl" begin
        for idx in idxs
            ...
            for q in qs
                ...
                @test ForwardDiff.gradient(f, p) ≈ FiniteDiff.finite_difference_gradient(f, p) 
                @test ForwardDiff.hessian(f, p) ≈ FiniteDiff.finite_difference_hessian(f, p) rtol = reltol 
            end
            fa(p) = ft(p, AdaptiveExpAndGram{eltype(p)}())
            @test ForwardDiff.gradient(fa, p) ≈ FiniteDiff.finite_difference_gradient(fa, p) 
            @test ForwardDiff.hessian(fa, p) ≈ FiniteDiff.finite_difference_hessian(fa, p) rtol = reltol
        end
    end
end
